...
export class PhrasePractice {
  ...
  public static nextReviewAfterSuccess(currentIntervalDays: number): number {
    const next = PhrasePractice.REVIEW_INTERVALS.find(
      (interval) => interval > currentIntervalDays
    );
    return (
      next ??
      PhrasePractice.REVIEW_INTERVALS[
        PhrasePractice.REVIEW_INTERVALS.length - 1
      ]
    );
  }
  ...
  public static masteryStateAfterReview(
    ...
    return isCorrect && intervalDays >= PhrasePractice.MASTERED_INTERVAL_DAYS
      ? "mastered"
      : "active";
  }
  ...
  static createNew(input: {
    ...
    return new PhrasePractice(
      ...
      input.category,
      input.meaningVi,
      input.meaningVi, // safeReviewPromptVi
      input.isSensitive,
      0, // intervalDays
      2.5, // easeFactor
      0, // repetitions
  ...
  recordReviewAttempt(isCorrect: boolean, score?: number, now = new Date()) {
    ...
    this._masteryState =
      this._intervalDays >= PhrasePractice.MASTERED_INTERVAL_DAYS
        ? "mastered"
        : "active";
  ...
  toPlainObject(): PhrasePracticePlain {
    return {
      id: this.id,
      userId: this.userId,
      source: this.source,

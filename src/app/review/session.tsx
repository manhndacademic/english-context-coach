...
export function ReviewSession({
  ...
  const handleComplete = () => {
    if (currentIndex < sessionPatterns.length - 1) {
      setCurrentIndex((prev) => prev + 1);
    } else {
      // Refresh the page to load fresh due patterns
      router.refresh();
      setCurrentIndex(sessionPatterns.length);
    }
  };
  ...
  const currentPattern = sessionPatterns[currentIndex];
    ...
    <div className="grid gap-4">
      <div className="grid gap-1.5 mb-2">
        <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-muted">
          ...
          <span>
            {currentIndex + 1} / {sessionPatterns.length} cụm từ
          </span>
        </div>
        <div className="h-2 bg-surface-strong border border-border rounded-full overflow-hidden shadow-inner">
          <div
            ...
            style={{
              width: `${(currentIndex / sessionPatterns.length) * 100}%`,
            }}
          />
        </div>
      </div>

...
function HistoryStatsGrid({
  lessonsCompleted,
  exercisesCompleted,
  patternCount,
  phrasePracticesCount,
  dueCount,
  masteredCount,
  avgRepetitions,
}: {
  ...
  patternCount: number;
  phrasePracticesCount: number;
  dueCount: number;
  masteredCount: number;
  avgRepetitions: string;
}) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 min-[960px]:grid-cols-7 gap-4">
      ...
      <StatCard label="Cụm từ lưu" value={phrasePracticesCount} />
      <StatCard
        label="Cần ôn tập"
        value={dueCount}
...
function RecentLessonsPanel({ recentLessons }: { recentLessons: any[] }) {
  return (
      ...
      <SectionCard.Body className="gap-3">
        {recentLessons.length === 0 ? (
          <p className="text-muted text-sm m-0 py-4 text-center">
            Bạn chưa hoàn thành bài học nào. Hãy dán văn bản mới ở trang chủ để
            bắt đầu!
          </p>
        ) : (
      ...
      <SectionCard.Body className="gap-4">
        {totalSavedCount === 0 ? (
          <p className="text-muted text-sm m-0 py-8 text-center">
            Chưa có mẫu lỗi hay cụm từ nào được ghi nhận. Hãy trả lời câu hỏi
            bài học để hệ thống lưu lại.
          </p>
        ) : (
            ...
            {[
              ...
              .map((item) => {
                if (item.isMistake) {
                  ...
                  const lessonsList =
                    lessonsMap[`${mistake.conceptKey}_${mistake.errorType}`] ||
                    [];
                  return (
                    <div
                      key={mistake.id}
  ...
  return (
    <PageLayout user={user}>
      ...
      <HistoryStatsGrid
        ...
        phrasePracticesCount={phrasePractices.length}
        dueCount={dueCount}
        masteredCount={masteredCount}
        avgRepetitions={avgRepetitions}
      />

      {/* Details Grid */}
      <div className="grid grid-cols-1 min-[960px]:grid-cols-[1fr_1.5fr] gap-6 items-start">
        <RecentLessonsPanel recentLessons={recentLessons} />
        <LearnedMistakesPanel
          patterns={patterns}
          phrasePractices={phrasePractices}

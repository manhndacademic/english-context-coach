    ...
    <main className="max-w-[1100px] mx-auto px-4 sm:px-6 py-10 flex flex-col gap-6">
      ...
      <section className="bg-surface border border-border rounded-lg p-5 sm:p-8 shadow-md grid gap-6">
        <div className="flex items-start gap-4 flex-col sm:flex-row border-b border-border pb-5">
          ...
          <div>
            <h1 className="text-2xl font-bold font-serif mb-1.5 text-text m-0">Cấu hình API Key cá nhân</h1>
            <p className="text-muted text-sm leading-relaxed m-0">
              Nhập API Key của riêng bạn từ Google AI Studio để tránh giới hạn lượt dùng chung của hệ thống.
            </p>
          </div>
        </div>
        ...
        <form action={saveUserApiKeyAction} className="grid gap-5">
          <div className="grid gap-2">
            ...
            <input
              ...
              id="apiKey"
              name="apiKey"
              placeholder={hasCustomKey ? "••••••••••••••••••••••••••••••••" : "Nhập API Key của bạn (AIzaSy...)"}
            ...
            {hasCustomKey ? (
              <span className="text-success text-xs flex items-center gap-1.5 font-medium mt-1">
                <ShieldCheck size={14} /> Bạn đã cấu hình API Key cá nhân. Nhập key mới để thay đổi hoặc bỏ trống và lưu để xóa key.
              </span>
            ) : (
              <span className="text-muted text-xs flex items-center gap-1.5 mt-1">
                <AlertCircle size={14} /> Bạn có thể lấy API Key miễn phí từ <a href="https://aistudio.google.com/" target="_blank" rel="noopener noreferrer" className="text-accent underline font-semibold">Google AI Studio</a>.
              </span>
            )}
          </div>
      ...
      <section className="bg-surface border border-border rounded-lg p-5 sm:p-8 shadow-sm grid gap-4">
        <h2 className="text-lg font-bold text-text flex items-center gap-2 m-0">
          <Sparkles size={18} className="text-accent" /> Tại sao nên dùng API Key cá nhân?
        </h2>
        <ul className="text-muted text-sm leading-relaxed grid gap-2 m-0 pl-5">
          <li><strong>Không lo quá tải</strong>: API Key chung của hệ thống có giới hạn số lượng request mỗi phút (Rate Limit). Dùng key riêng giúp bạn chạy mượt mà bất cứ lúc nào.</li>
          <li><strong>Bảo mật tối đa</strong>: API Key của bạn sẽ được mã hóa an toàn bằng thuật toán đối xứng <code>AES-256-GCM</code> trước khi lưu vào cơ sở dữ liệu và chỉ được giải mã tạm thời khi gửi request đến Google.</li>
          <li><strong>Hoàn toàn miễn phí</strong>: Google AI Studio cung cấp gói Free Tier với giới hạn khá lớn cho việc học tập cá nhân.</li>
        </ul>
      </section>
    </main>
  );

...
/**
 * Word-level diff utility using Myers LCS algorithm.
 * Tokenizes by whitespace then diffs at the word level.
 * Designed for comparing Vietnamese translation answers where word-level
 * granularity is more readable than character-level.
 *
 * Re-uses DiffSpan type: each span's `text` is a space-joined sequence of words.
 *
 * If corrected is null/undefined/empty or equals original, returns a single "equal" span.
 */
export function computeWordDiff(
  ...
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && a[i - 1] === b[j - 1]) {
      ops.push({ type: "equal", word: a[i - 1] });
      i--;
      j--;
    } else if (j > 0 && (i === 0 || table[i][j - 1] >= table[i - 1][j])) {
  ...
  // Merge consecutive ops of the same type into spans (space-joined words)
  const spans: DiffSpan[] = [];
  for (const op of ops) {
    const last = spans[spans.length - 1];
    if (last && last.type === op.type) {
      ...
      spans.push({ type: op.type, text: op.word });
    }
  }

  // Add space separators between spans for readability
  return spans.map((span, idx) => {
    const needsLeadingSpace = idx > 0 && spans[idx - 1].type !== span.type;
    return needsLeadingSpace ? { ...span, text: " " + span.text } : span;
  });
}

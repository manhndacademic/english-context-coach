...
export function LandingTabs() {
  ...
  const tabs = {
    dev: {
      ...
      example: "This endpoint is deprecated and will be removed in a future release.",
      trap: "deprecated",
      trapWrong: "Bị phản đối / Bị chê",
      trapCorrect: "Không khuyến khích dùng nữa, sắp bị gỡ bỏ (hạn chế dùng cho code mới)"
    },
    business: {
    ...
    academic: {
      ...
      example: "The test coverage was improved, albeit at the cost of execution speed.",
      trap: "albeit",
    ...
    <div className="bg-surface border border-border rounded-lg overflow-hidden shadow-md">
      ...
      <div className="p-4 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 items-center">
        <div className="grid gap-4">
          <h3 className="text-xl md:text-2xl font-extrabold m-0 text-text">{current.title}</h3>
          <p className="text-sm leading-relaxed text-muted m-0">{current.copy}</p>
        ...
        <div className="min-w-0 w-full">
          <div className="bg-surface-strong border border-border rounded-md p-5 font-serif text-sm md:text-base leading-relaxed shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)]">
            <strong style={{ display: "block", fontSize: "12px", color: "var(--muted)", textTransform: "uppercase", marginBottom: "8px", letterSpacing: "0.05em" }}>Ví dụ thực tế</strong>
            <p style={{ margin: "0 0 16px", fontStyle: "italic", fontSize: "16px", color: "var(--accent-strong)" }}>
              &quot;{current.example}&quot;
            </p>
            ...
            <div className="w-full overflow-x-auto mt-4 rounded border border-border">
              <table className="w-full border-collapse text-sm min-w-[500px]">
                <thead>
                  <tr>
                    <th className="bg-surface-strong font-bold text-text text-left p-2.5 border-b border-border" style={{ width: "30%" }}>Cụm từ</th>
                    <th className="bg-surface-strong font-bold text-danger text-left p-2.5 border-b border-border" style={{ width: "35%" }}>Dịch từng chữ</th>
                    <th className="bg-surface-strong font-bold text-success text-left p-2.5 border-b border-border" style={{ width: "35%" }}>Nghĩa ngữ cảnh</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="p-2.5 border-b border-border text-left font-bold">{current.trap}</td>
                    <td className="p-2.5 border-b border-border text-left text-danger line-through">{current.trapWrong}</td>
                    <td className="p-2.5 border-b border-border text-left font-semibold text-success">{current.trapCorrect}</td>
                  </tr>
                </tbody>
              </table>
            </div>

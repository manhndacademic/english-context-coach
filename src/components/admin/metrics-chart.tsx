  ...
  if (data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[200px] text-center bg-surface-strong/30 rounded-md border border-dashed border-border p-4">
        <p className="text-sm font-semibold text-muted m-0">Chưa có dữ liệu thống kê theo ngày</p>
      </div>
    );
  }
  ...
  if (!mounted) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-layout-gap">
        <div className="bg-surface border border-border rounded-lg p-5 shadow-sm min-w-0 h-[280px] animate-pulse bg-surface-strong/10" />
        <div className="bg-surface border border-border rounded-lg p-5 shadow-sm min-w-0 h-[280px] animate-pulse bg-surface-strong/10" />
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-layout-gap">
      <div className="bg-surface border border-border rounded-lg p-5 shadow-sm min-w-0">
        <h3 className="text-sm font-bold text-muted uppercase tracking-wider mb-4 mt-0">Tần suất request (30 ngày gần đây)</h3>
        <div className="h-[220px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={formattedData} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
              ...
              />
              <Area type="monotone" dataKey="requests" stroke="#059669" strokeWidth={2} fill="url(#requestsGradient)" dot={{ fill: "#059669", r: 3 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-lg p-5 shadow-sm min-w-0">
        <h3 className="text-sm font-bold text-muted uppercase tracking-wider mb-4 mt-0">Chi phí tích lũy (30 ngày gần đây)</h3>
        <div className="h-[220px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={formattedData} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
              <defs>
              ...
              />
              <Area type="monotone" dataKey="costUsd" stroke="#d97706" strokeWidth={2} fill="url(#costGradient)" dot={{ fill: "#d97706", r: 3 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>

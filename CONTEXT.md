...
English Context Coach helps Vietnamese learners understand English in context, write more naturally for their cultural situation, and practice repeated misunderstandings.

## Language

**SourceText**:
The English material a Lesson is built on. In read mode, SourceText is the original text the learner pasted. In write mode, SourceText is the AppSuggestion (the improved version generated by the app). In diff mode, SourceText is the corrected version the learner provided. SourceText is always the "right" version to learn from. Use natural learner-facing copy such as "source text" or "English source text" instead of the PascalCase term in UI text.
_Avoid_: Text, InputText, Article

**DraftText**:
The learner's own English writing before correction. In write mode, the app generates an AppSuggestion from the DraftText; in diff mode, the learner provides the corrected version themselves. DraftText is compared against SourceText to produce CorrectionItems. A Lesson may or may not have a DraftText — when absent, the Lesson operates in read mode.
_Avoid_: Original text, raw text, user input

**CorrectionItem**:
A single difference between a DraftText and its SourceText, representing one correction the learner needs to internalize. Each CorrectionItem includes a culturalNoteVi explaining the cultural or register reasoning behind the change. Each CorrectionItem generates a scaffolded exercise group (recognize → guided → produce). CorrectionItems are the primary learning unit in write and diff modes.
_Avoid_: Diff, change, fix, correction

**CommunicationContext**:
The composite of a DocumentType and a Formality that describes the situation the text is written for. CommunicationContext calibrates the strictness of AppSuggestion corrections, the cultural depth of explanations, and the domain vocabulary the AI prioritizes. A Lesson stores its CommunicationContext. The app auto-detects both axes from the pasted text; the learner can override either axis and re-analyze.
_Avoid_: Mode, input mode, text type, register

**DocumentType**:
The kind of written artifact the learner is producing or reading. DocumentType determines which domain vocabulary, structural conventions, and phrasing patterns the AI should teach. New DocumentTypes can be added without changing Formality. Stored in the DB as the existing `text_type` column (extended enum). Values: `email`, `chat_message`, `ticket`, `code_review`, `technical_doc`, `meeting_notes`, `general`. Legacy values `work_message`, `article`, `academic`, `unknown` remain valid for backward compatibility and future expansion.
_Avoid_: Text type, category, channel

**Formality**:
The register or tone level of the text, independent of DocumentType. Formality determines how strict the AI is about politeness, hedging, directness, and cultural conventions. The same DocumentType can appear at different Formality levels. Values: `formal` (client-facing, proposals, public documentation), `semi_formal` (internal team docs, tickets, PR comments), `casual` (team chat, quick DMs).
_Avoid_: Tone, register, level

**AppSuggestion**:
The app-generated improved version of a learner's DraftText. An AppSuggestion rewrites the DraftText for naturalness, cultural appropriateness, and correctness according to the selected CommunicationContext. The learner can accept or reject each individual correction within the AppSuggestion. An accepted AppSuggestion becomes the SourceText for the Lesson.
_Avoid_: AI correction, auto-fix, rewrite

**LearningFlow**:
One of three ways a Lesson is generated: write flow (learner pastes their own text, app generates an AppSuggestion), read flow (learner pastes text from someone else, app explains meaning and context), or diff flow (learner pastes both their draft and an externally-corrected version). The app auto-detects the appropriate LearningFlow but the learner can override it.
_Avoid_: Mode, input mode, learning mode

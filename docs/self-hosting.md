...
| `GEMINI_MAX_OUTPUT_TOKENS_ANALYSIS` | Max output token limit for lesson analysis.           | Defaults to `8192` (Gemini's maximum output limit).                                                                                                    |
| `GEMINI_MAX_OUTPUT_TOKENS_EXERCISE` | Max output token limit for exercise generation.       | Defaults to `2200`.                                                                                                                                    |
| `GEMINI_MAX_OUTPUT_TOKENS_GRADING`  | Max output token limit for attempt grading.           | Defaults to `4096`.                                                                                                                                    |
| `GEMINI_MAX_OUTPUT_TOKENS_REPAIR`   | Max output token limit for JSON response repair.      | Defaults to `4096`.                                                                                                                                    |
